...
import org.jbpm.ui.chatflow.factory.ElementCreationFactory;
import org.jbpm.ui.model.ElementType;


public class ChatFlowPaletteRoot extends PaletteRoot {
	
	private ChatFlowEditor editor;
	
	public ChatFlowPaletteRoot(ChatFlowEditor editor) {
		this.editor = editor;
		addControls();	
	}
	...
	private boolean include(String name) {
		String[] types = {"start-state", "end-state", "node", "fork", "join", "decision",  "chatpage", "nowaitchatpage", "nowaitnode"};
		for (int i = 0; i < types.length; i++) {
			if (types[i].equals(name)) return true;
		}
		return false;
	}
	...
	private PaletteEntry createEntry(ElementType elementType) {
		...
		if ("node".equals(type) && include(name)) {
